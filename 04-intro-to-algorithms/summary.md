...
todo: react todo app + context (codesandbox), backend (weather app), cypress test automation, mysql, mongodb
...
> bubble sort: kita comparekan element bersebelahan. kalau element sekarang besar dari element sebelah, kita swap.

**How it works:**

> outer loop repeat (main array), from left to second-last length of main array. inner loop repeat (subarray), from left to third-last length of main array. remember: we are comparing neighboring elements so, if current element of subarray > next element of subarray. then we swap the current element with the next element, and vice versa.
...
> repeat, from 1st element to last element. remember: insertion sort compare current element with the 
