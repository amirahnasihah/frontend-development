...
3. Inside if block, penting kita nak kena cari adalah Pivot Index(pi). Kita create partition function untuk nak tahu kat mana pivot duduk lepas kita asingkan elements. (First cycle, range would be from 0 to end. Partition function should return the position of pivot after partition). the `quickSort` function is called with the array array, and the values `0` (which is the minimum index) and `array.length - 1` (which is the maximum index) are passed as arguments.
